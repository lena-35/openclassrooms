# 🤖 Projet : Formation Openclassrooms

## 📝 Présentation du Projet
Ce dépôt centralise l'ensemble de ma formation technique.
L'objectif est de maîtriser et d'allier la programmation bas niveau, l'intelligence artificielle (RAG), la gestion d'infrastructure, le développement web et les bases de données pour créer un écosystème logiciel complet.

---

## 📅 Calendrier de Formation
- **Début :** 31/03/2026
- **Fin estimée :** xx/xx/2026
- **Rythme :** x heures / jour (du lundi au vendredi)

---

## 🗂️ Structure du Programme (328h)

| Ordre | Module | Durée | Statut |
| :--- | :--- | :---: | :--- |
| 01 | [Gestion de Projet](./01_Gestion_Projet/) | 42h | ⏳ À venir |
| 02 | [Algorithmes & Qualité code](./02_Algorithmes_Qualite/) | 16h | ⏳ À venir |
| 03 | [Programmation C / C++](./03_Prog/) | 46h | ⏳ À venir |
| 04 | [Langages Python / C#](./04_Langage/) | 26h | ⏳ À venir |
| 05 | [Qualité du Code Python](./05_Qualite_Code_Python/) | 4h | ⏳ À venir |
| 06 | [Intelligence Artificielle](./06_IA/) | 40h | ⏳ À venir |
| 07 | [Design UX/UI](./07_Design/) | 10h | ⏳ À venir |
| 08 | [Web & Bases de données](./08_Web_Data/) | 47h | ⏳ À venir |
| 09 | [Infrastructure & Réseau](./09_Infra/) | 18h | ⏳ À venir |
| 10 | [DevOps (Docker)](./10_DevOps/) | 20h | ⏳ À venir |
| 11 | [Cybersécurité](./11_Cyber/) | 20h | ⏳ À venir |
| 12 | [Industrie (BIM/AutoCAD)](./12_Indus/) | 21h | ⏳ À venir |
| 13 | [Bureautique (Excel)](./13_Bureautique/) | 18h | ⏳ À venir |

---

## 🛠️ Stack Technique & Outils
* **Gestion de projet :** Cahier des charges fonctionnel, Spécifications techniques, Méthode Agile
* **Qualité :** Algorithmes, Tests, Maintenabilité
* **Langages :** C, C++, Python, C#, SQL, HTML/CSS
* **IA :** RAG (Retrieval Augmented Generation), LangChain, Jupyter Notebooks
* **Infrastructure :** Linux, Docker, GitHub Actions
* **Environnement :** GitHub Codespaces (Browser-based)
* **Design :** Figma, AutoCAD

---

## 🚀 Comment naviguer ?
* Chaque dossier contient son propre fichier `README.md` avec le détail du cours.
* Chaque dossier contient son propre fichier `Notes.md` avec le suivi du cours.

> **Note :** Ce projet est réalisé en mode "Zéro Installation" via le navigateur Opera.